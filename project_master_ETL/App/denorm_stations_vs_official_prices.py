...
def merge_denorm_stations_vs_official_prices(year_to_load = None, drop_mongo_collections = None):
    ...
    if drop_mongo_collections == "true":
        ...
        mongo_manager.drop_mongo_collections(bdd = "denormalization", collections= ["denorm_stations_vs_official_prices"])
    ...
    if df_official_oils.empty:
        return None
    denorm_stations_vs_official_prices = transform_merge_stations_vs_official_prices(df_denorm_stations, df_official_oils)
...
def extract_denorm_stations_prices_from_mongo(start_date_to_load, end_date_to_load):
    ...
    df_denorm_stations = mongo_manager.get_filtered_datas_from_one_collection(start_date_to_load, end_date_to_load, db_name="denormalization", collection="denorm_stations_prices")
    if df_denorm_stations.empty:
...
def transform_merge_stations_vs_official_prices(df_denorm_stations, df_official_oils):
    print("[INFO] Start transform_merge_stations_vs_official_prices")
    df_denorm_stations['Date'] = pd.to_datetime(df_denorm_stations['Date'])
    df_official_oils['Date'] = pd.to_datetime(df_official_oils['Date'])

    # merge the 2 df together
    df_merge = pd.merge(df_official_oils, df_denorm_stations, on='Date', how='outer')
...
def load_denorm_stations_vs_official_prices_to_mongo(denorm_stations_vs_official_prices):
    ...
    # Save df to Mongo
    result = mongo_manager.load_datas_to_mongo(denorm_stations_vs_official_prices, bdd="denormalization",collection="denorm_station_vs_official_prices", index=["Date"])
    if result:
