...
def list_S3_contents(bucket_name=BUCKET_NAME, prefix=None):
    if prefix:
    ...
    if not contents:
        prefix_msg = f"under '{prefix}'" if prefix else "in the entire bucket"
        print(f"[INFO] No files found {prefix_msg} in bucket '{bucket_name}'.")
        return []
    simplified_contents = [
        {
            "Key": obj["Key"],
    ...
    prefix_msg = f"under prefix '{prefix}'" if prefix else "in the bucket"
    print(f"[INFO] Files {prefix_msg}:")
...
def download_file_from_s3_to_path(object_name, out_path, bucket=BUCKET_NAME):
    ...
    complete_dump_path = os.path.join(out_path, file_name)
